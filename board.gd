...
func get_pawn_moves():
	...
	pos = seleted_piece + Vector2(direction.x, 1)
	if is_valid_position(pos):
	...
	pos = seleted_piece + direction * 2
	if is_first_move && is_empty(pos) && is_empty(seleted_piece + direction) : _moves.append(pos)
	return _moves
